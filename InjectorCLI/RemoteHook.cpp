	...
	*/
	...
	if (onceNativeCallContinue == false)
	{
		...
		else
		{
			...
			wsprintfA(message, "%p len %d last error %d", sysWowSpecialJmpAddress, minDetourLen, GetLastError());
			MessageBoxA(0, "Failed to unprotect WOW64 gateway", message, MB_ICONERROR);
		}
